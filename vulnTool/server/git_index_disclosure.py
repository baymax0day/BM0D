...
def get_plugin_info():
    plugin_info = {
        "name": "git代码泄露",
        "info": ".git代码泄露,可使用githack工具",
        "exp_url": "https://blog.csdn.net/u011066706/article/details/51175761;https://github.com/lijiejie/GitHack",
        "docker": "",
        "other": ""
    }
    return plugin_info
...
def check(ip, port, timeout):
    try:
        url = "http://"+ip+":" + port
        res = requests.get(url+"/.git",timeout)
        if res.status_code == 200:
            return u"%s 存在git目录,可使用gitHack工具" % url
        res = requests.get(url + "/.git/config", timeout)
        if res.status_code == 200:
...
def parse(filename, pretty=True):
    with open(filename, "rb") as o:
        ...
        check_err(index["signature"] == "DIRC", "Not a Git index file")

        # 4-byte version number
        index["version"] = read("I")
        check_err(index["version"] in {2, 3},
        ...
        for n in range(index["entries"]):
            ...
            if entry["extended"] and (index["version"] == 3):
                ...
                # check_err(not used, "Expected unused bits in extra-flags")
                entrylen += 2
            ...
            nuls = f.read(padlen)
            check_err(set(nuls) == set(['\x00']), "padding contained non-NUL")
